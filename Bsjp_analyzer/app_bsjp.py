...
# --- FUNGSI UTAMA APLIKASI BSJP (Hanya jalan kalau sudah login) ---
def main_app():
...
if not st.session_state.logged_in:
    # Desain halaman login di tengah
                ...
                if not user_input or not pass_input:
                    ...
                    with st.spinner("Memverifikasi data langganan..."):
                        try:
                            ...
                            # Gunakan parameter spreadsheet dengan link Google Sheets kamu nanti
                            users_df = conn.read()
                            
                            # Mengecek kecocokan Username
                            user_data = users_df[users_df['Username'] == user_input]
                            
                            if not user_data.empty:
                                # Mengecek kecocokan Password (diubah jadi string untuk jaga-jaga angka)
                            ...
                            st.error(f"Gagal terhubung ke Database. Pastikan pengaturan Secrets sudah benar.")
else:
